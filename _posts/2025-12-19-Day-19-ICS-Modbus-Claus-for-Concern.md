...
date:   2025-12-19
categories: welcome
image: Day19-banner.png
---

## Claus for Concern: My Dive into ICS and Modbus on TryHackMe

Ah, the holiday season – a time for cheer, twinkling lights, and... hacking industrial control systems? That's exactly the vibe I got when I stumbled upon TryHackMe's "ICS/Modbus - Claus for Concern" room. As part of what I suspect is their Advent of Cyber 2025 series (judging by the "aoc2025" in the URL), this medium-difficulty challenge clocks in at about 60 minutes and throws you into a festive yet fiendish scenario involving compromised drone deliveries. Picture this: Santa's high-tech workshop has been infiltrated by the nefarious King Malhare, who's swapped out Christmas gifts for chocolate eggs. It's up to you, the cyber elf detective, to restore order using your wits and a deep dive into the Modbus protocol. What started as a casual evening of CTF fun turned into an eye-opening lesson on the vulnerabilities lurking in real-world industrial systems. Let me take you through my journey, step by step, without spoiling the flags – because where's the fun in that?

#### The Setup: A Festive Cyber Nightmare

I joined the room via the provided URL and was immediately greeted by a whimsical backstory. The TBFC Drone Delivery System – think Santa's sleigh on steroids – relies on Supervisory Control and Data Acquisition (SCADA) systems to monitor and manage operations. At the heart of it all are Programmable Logic Controllers (PLCs), those unsung heroes of automation that read sensors, flip switches, and keep everything humming. But here's the twist: the bad guy has exploited Modbus, a widely used communication protocol in industrial environments, to tamper with the setup. No authentication, no encryption – just raw access over TCP port 502. It's like leaving your front door wide open during a blizzard.

The room's learning objectives hit home right away: understand SCADA as the big-picture monitoring hub, grasp how PLCs handle real-time control, and get cozy with Modbus basics. Modbus organizes data into types like coils (for digital on/off states) and holding registers (for analog values). Addresses start at zero, and it's all client-server chit-chat. In this scenario, key registers like HR0 dictate package types (0 for gifts, 1 for eggs – sneaky!), while coils control things like security checks and logging. The compromise? Disabled verifications, falsified data, and a sneaky "eggsploit" signature in HR4 set to 666. Oh, and there's a trap: mess with the wrong thing first, and you trigger a self-destruct sequence that dumps inventory into the ocean. Holiday chaos, indeed.

#### Reconnaissance: Peeking Behind the Curtain

My adventure kicked off with reconnaissance, the bread and butter of any pentest. Using good old Nmap, I scanned for open ports – 22 for SSH, 80 for web, and that telltale 502 for Modbus. Port 80 led to a CCTV feed, which was a brilliant touch: watching drones deliver eggs instead of gifts visually hammered home the breach. It felt like being in a spy movie, confirming the compromise without touching a single register.

From there, I shifted to interacting with Modbus itself. The room guides you toward Python's pymodbus library – a lifesaver for scripting connections. Reading holding registers revealed the tampering: package types flipped, signatures left like digital graffiti. Coils showed disabled logging and verification, plus an active protection mechanism on C11. It was a puzzle – figure out the compromised state, generate a "reconnaissance report" by querying specific values. This part taught me how exposed ICS can be; in the real world, legacy systems often run Modbus without modern safeguards, making them prime targets for supply chain attacks or ransomware.

#### The Remediation Rollercoaster: Dodging Traps and Restoring Joy

Here's where the narrative ramps up – remediation. You can't just barge in and flip HR0 back to gifts; that protection coil (C11) is a booby trap. Enable it, and bam – C15 arms a 30-second countdown, activating C12 to send everything to "Zone 10" (spoiler: that's the deep blue sea). My first attempt? Yeah, I triggered it. Inventory gone, lesson learned: order matters in ICS. Disable protections first, then make changes, and finally re-enable safeguards like inventory checks (C10) and auditing (C13). Once done, a flag appears in holding registers starting at address 20 – your reward for saving Christmas.

The thrill was in the logic: it's not just hacking; it's understanding the system's interdependencies. One wrong move, and you're dealing with cascading failures. This mirrors real ICS incidents, like Stuxnet or colonial pipeline hacks, where protocol weaknesses lead to physical disruptions. The room emphasizes safe practices – no brute force, just precise reads and writes.

#### Lessons from the North Pole: Why This Matters

Wrapping up, "Claus for Concern" isn't just a game; it's a wake-up call. ICS powers everything from power grids to factories, and Modbus's simplicity is its Achilles' heel – no built-in security means network exposure can spell disaster. I walked away appreciating tools like pymodbus for ethical testing and the importance of segmentation, encryption, and monitoring in production environments. If you're new to OT (Operational Technology) security, this room is a perfect entry point – no heavy prerequisites beyond basic networking and Python.

Would I recommend it? Absolutely. It's engaging, thematic, and packs a punch in under an hour. If you're into cybersecurity with a side of holiday spirit, deploy that VM and give it a go. Who knew saving Santa could teach you so much about industrial protocols? Ho ho hack!


## Task 1 - Introduction


 ![](https://tryhackme-images.s3.amazonaws.com/user-uploads/63c131e50a24c3005eb34678/room-content/63c131e50a24c3005eb34678-1763818313841.png)
...
Before moving forward, review the questions in the connection card shown below:
 ...
 ![VM and AttackBox connection card](https://tryhackme-images.s3.amazonaws.com/user-uploads/63c131e50a24c3005eb34678/room-content/63c131e50a24c3005eb34678-1763818542725.png)
